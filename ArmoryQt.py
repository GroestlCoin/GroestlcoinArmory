...
class ArmoryMainWindow(QMainWindow):
   ...
   def __init__(self, parent=None, splashScreen=None):
      ...
      if USE_TESTNET:
         self.setWindowTitle('Groestlcoin Armory - Wallet Management [TESTNET] dlgMain')
         ...
         self.setWindowTitle('Groestlcoin Armory - Wallet Management')
      ...
      # We only need a single connection to groestlcoind since it's a
...
#!!!GRS      self.setupAnnouncementFetcher()
      ...
      if self.abortLoad:
         LOGWARN('Groestlcoin Armory startup was aborted.  Closing.')
         os._exit(0)
      ...
      # Setup system tray and register "groestlcoin:" URLs with the OS
...
#!!!GRS      self.mainDisplayTabs.addTab(self.tabAnnounce,  tr('Announcements'))
      ...
      actExportLog   = self.createAction('Export &Log File...',
                                         self.exportLogFile)
      actCloseApp    = self.createAction('&Quit Groestlcoin Armory', self.closeForReal)
      ...
      actTroubleshoot = self.createAction(tr('Troubleshooting Groestlcoin Armory'), execTrouble)
      ...
      actPrivacyPolicy = self.createAction(tr('Groestlcoin Armory Privacy Policy'), self.showPrivacyGeneric)
...
#GRS      self.menusList[MENUS.Help].addAction(actPrivacyPolicy)
   ...
   def getExtraEntropyForKeyGen(self):
      ...
      return SecureBinaryData(HMAC256('Groestlcoin Armory Entropy', allEntropy))
   ...
   def setupSystemTray(self):
      ...
      self.sysTray.setToolTip('Groestlcoin Armory' + (' [Testnet]' if USE_TESTNET else ''))
      ...
      actShowArmory = self.createAction('Show Groestlcoin Armory', self.bringArmoryToFront)
      actSendBtc    = self.createAction('Send Groestlcoins', traySend)
      actRcvBtc     = self.createAction('Receive Groestlcoins', trayRecv)
      actClose      = self.createAction('Quit Groestlcoin Armory', self.closeForReal)
         ...
         try:
            ...
            if 'armory' in val.lower():
               if val.lower()==modulepathname.lower():
                  LOGINFO('Groestlcoin Armory already registered for current user.  Done!')
                  return
               else:
            ...
            try:
               ...
               val,code = QueryValueEx(registryKey, '')
               if 'armory' in val.lower():
                  LOGINFO('Groestlcoin Armory already registered at admin level.  Done!')
                  return
               else:
   ...
   def warnNewUSTXFormat(self):
      if False: #!!!P GRS             not self.getSettingOrSetDefault('DNAA_Version092Warn', False):
         reply = MsgBoxWithDNAA(self, self, MSGBOX.Warning, tr("Version Warning"), tr("""
            ...
            versions of Groestlcoin Armory before 0.92.
            <br><br>
...
#                      ['announce',  self.updateAnnounceTab],
   ...
   def processAlerts(self):
      # display to the user any alerts that came in through the groestlcoin
      # network
      ...
      try:
         ...
         LOGERROR("failed to process alerts from groestlcoind")
         return
   ...
   def processDownloads(self, txt):
      try:
         ...
         if not 'Armory' in self.downloadLinks:
            LOGWARN('No Groestlcoin Armory links in the downloads list')
         else:
            maxVer = 0
            self.versionNotification = {}
            for verStr,vermap in self.downloadLinks['Armory'].iteritems():
               ...
               if dlVer > maxVer:
                  ...
                  shortDescr = tr('Groestlcoin Armory version %s is now available!') % verStr
            ...
            if 'ArmoryTesting' in self.downloadLinks:
               for verStr,vermap in self.downloadLinks['ArmoryTesting'].iteritems():
                  ...
                  if dlVer > maxVer:
                     ...
                     shortDescr = tr('Groestlcoin Armory Testing version %s is now available!') % verStr
                     notifyID = binary_to_hex(hash256(shortDescr)[:4])
                     self.versionNotification['UNIQUEID'] = notifyID
                     self.versionNotification['VERSION'] = '0'
                     self.versionNotification['STARTTIME'] = '0'
         ...
         if not 'Satoshi' in self.downloadLinks:
            ...
            try:
               ...
               # This is to detect the running versions of Groestlcoin-Qt/groestlcoind
   ...
   def processNotifications(self, txt):
      ...
      # well as every 30 min while Groestlcoin Armory is running.  All notifications are
   ...
   def setupNetworking(self):
      ...
      if CLI_OPTIONS.interport > 1:
         try:
            ...
            LOGWARN('Socket already occupied!  This must be a duplicate Groestlcoin Armory')
            QMessageBox.warning(self, tr('Already Open'), tr("""
               Groestlcoin Armory is already running!  You can only have one Groestlcoin Armory open
   ...
   def manageBitcoindAskTorrent(self):
      ...
      if not satoshiIsAvailable():
         reply = MsgBoxCustom(MSGBOX.Question, tr('BitTorrent Option'), tr("""
            ...
            yourself (Groestlcoin-Qt or groestlcoind).  <u>Normally</u>, you should
            start the Groestlcoin software first and wait for it to synchronize
            with the network before starting Groestlcoin Armory.
            <br><br>
            ...
            Click "Cancel" below, then close Groestlcoin Armory and start Groestlcoin-Qt
            (or groestlcoind).  Do not start Groestlcoin Armory until you see a green checkmark
         ...
         if reply:
            ...
            QMessageBox.warning(self, tr('Synchronize'), tr("""
               ...
               Groestlcoin Armory until it is finished.  If you are running Groestlcoin-Qt,
               you will see a green checkmark in the bottom-right corner,
               when it is time to open Groestlcoin Armory again."""), QMessageBox.Ok)
            return False
   ...
   def startBitcoindIfNecessary(self):
      ...
      if not self.doAutoBitcoind:
         LOGWARN('Tried to start groestlcoind, but ManageSatoshi==False')
         return False

      if satoshiIsAvailable():
         LOGWARN('Tried to start groestlcoind, but satoshi already running')
         return False

      self.setSatoshiPaths()
      TheSDM.setDisabled(False)

      torrentIsDisabled = True #!!!GRS self.getSettingOrSetDefault('DisableTorrent', False)
      ...
      try:
         ...
         # path the executable.  That dir tree will be searched for groestlcoind
...
#!!!GRS      TheTDM.setSatoshiDir(self.satoshiHomePath)
   ...
   def getSingletonConnectedNetworkingFactory(self):
      if not self.SingletonConnectedNetworkingFactory:
         ...
         def showOfflineMsg():
            ...
            try:
               self.sysTray.showMessage('Disconnected', 'Connection to Groestlcoin-Qt ' \
                                        'client lost!  Groestlcoin Armory cannot send nor ' \
         ...
         def showOnlineMsg():
            ...
            try:
               if self.connectCount>0:
                  self.sysTray.showMessage('Connected', 'Connection to Groestlcoin-Qt ' \
                                           're-established', \
   ...
   def parseUriLink(self, uriStr, clickOrEnter='click'):
      ...
      if TheBDM.getState() in (BDM_OFFLINE,BDM_UNINITIALIZED):
         LOGERROR('%sed "groestlcoin:" link in offline mode.' % ClickOrEnter)
         self.bringArmoryToFront()
      ...
      if len(uriDict)==0:
         warnMsg = ('It looks like you just %sed a "groestlcoin:" link, but '
      ...
      if not uriDict.has_key('address'):
         QMessageBox.warning(self, 'The "groestlcoin:" link you just %sed '
            'does not even contain an address!  There is nothing that '
            'Groestlcoin Armory can do with this link!' % clickOrEnter, QMessageBox.Ok)
         LOGERROR('No address in "groestlcoin:" link!  Nothing to do!')
         return {}
      ...
      if theAddrByte!=-1 and not theAddrByte in [ADDRBYTE, P2SHBYTE]:
         ...
         QMessageBox.warning(self, 'Wrong Network!', \
            'The address for the "groestlcoin:" link you just %sed is '
...
#!!!P GRS      self.updateAnnounceTab()  # make sure satoshi version info is up to date
   ...
   def confirmSweepScan(self, pybtcaddrList, targAddr160):
      ...
      if TheBDM.getState() in (BDM_OFFLINE, BDM_UNINITIALIZED):
         ...
         msgConfirm = ( \
            'Groestlcoin Armory must scan the global transaction history in order to '
            'find any groestlcoins associated with the %s you supplied. '
            'Groestlcoin Armory will go into offline mode temporarily while the scan '
         ...
         def checkForTxInBDM(val):
            ...
            if not TheBDM.bdv().getTxByHash(newTxHash).isInitialized():
               ...
               QMessageBox.warning(self, tr('Transaction Not Accepted'), tr("""
                  ...
                  appear to have been accepted by the Groestlcoin network yet. 
   ...
   def warnNoImportWhileScan(self):
      ...
      if not self.usermode==USERMODE.Standard:
         extraMsg = ('<br><br>' + \
                     ...
                     'starting Groestlcoin Armory in offline mode (--offline), and '
                     'perform the import before switching to online mode.'))
      QMessageBox.warning(self, tr('Groestlcoin Armory is Busy'), \
         tr('Wallets and addresses cannot be imported while Groestlcoin Armory is in '
   ...
   def showLedgerTx(self):
      ...
      if pytx==None:
         ...
         'Groestlcoin Armory\'s database.  This is unusual...', QMessageBox.Ok)
         return
   ...
   def uriSendBitcoins(self, uriDict):
      # Because Groestlcoin-Qt doesn't store the message= field we have to assume
      ...
      descrStr = ''
      descrStr = ('You just clicked on a "groestlcoin:" link requesting groestlcoins '
      ...
      if len(self.walletMap)==0:
         reply = QMessageBox.information(self, 'No Wallets!', \
            'You just clicked on a "groestlcoin:" link to send money, but you '
   ...
   def minimizeArmory(self):
      LOGDEBUG('Minimizing Groestlcoin Armory')
      self.hide()
      self.sysTray.show()
...
#GRS      if self.logFilePrivacyWarning(wCancel=True):
   ...
   def logFilePrivacyWarning(self, wCancel=False):
      return MsgBoxCustom(MSGBOX.Warning, tr('Privacy Warning'), tr("""
         ...
         Only enough data is there to help the Groestlcoin Armory developers
         track down bugs in the software, but it may still be considered
         sensitive information to some users.
         <br><br>

         Please do not send the log file to the Groestlcoin Armory developers if you
      ...
      if TheSDM.getSDMState() == 'GroestlcoindExeMissing':
         ...
         if satoshiIsAvailable():
            result = QMessageBox.warning(self, tr('Still Running'), tr("""
               'Groestlcoin-Qt is still running.  Groestlcoin Armory cannot start until
   ...
   def setupDashboard(self):
      ...
      # to help the user install groestlcoind.
      ...
      self.dashBtns[DASHBTNS.Close][LBL] = QRichLabel( \
           'Stop existing Groestlcoin processes so that Groestlcoin Armory can open its own')
      ...
      self.dashBtns[DASHBTNS.Settings][LBL]  = QRichLabel( \
           'Open Groestlcoin Armory settings window to change Groestlcoin software management')


      self.dashBtns[DASHBTNS.Browse][TTIP] = self.createToolTipWidget( \
           'Will open your default browser to http://www.groestlcoin.org where you can '
      ...
      self.dashBtns[DASHBTNS.Settings][TTIP] = self.createToolTipWidget(
           'Change Groestlcoin-Qt/groestlcoind management settings or point Groestlcoin Armory to '
           'a non-standard Groestlcoin installation')
      self.dashBtns[DASHBTNS.Close][TTIP] = self.createToolTipWidget( \
           'Groestlcoin Armory has detected a running Groestlcoin-Qt or groestlcoind instance and '
   ...
   def openDLArmory(self):
      dl,cl = self.getDownloaderData()
      if not dl is None and not cl is None:
         UpgradeDownloaderDialog(self, self, 'Groestlcoin Armory', dl, cl).exec_()
      ...
      # Show CoreBTC updates
         ...
         #self.btnSecureDLSatoshi.setVisible(False)
            ...
            #self.lblSatoshiSWVersion.setText(tr(""" You are running
               #core Groestlcoin software version %s""") % self.satoshiVersions[0])
         #else:
            #self.lblSatoshiSWVersion.setText(tr("""No information is
            #available for the core Groestlcoin software"""))
      except:
   ...
   def closeExistingBitcoin(self):
      ...
      QMessageBox.warning(self, 'Not Found', \
         'Attempted to kill the running Groestlcoin-Qt/groestlcoind instance, '
   ...
   def GetDashFunctionalityText(self, func):
      ...
      if func.lower() == 'scanning':
         ...
         'okay to create and distribute payment addresses while Groestlcoin Armory is offline, '
         'you just won\'t be able to verify those payments until the next time '
         'Groestlcoin Armory is online.')
         ...
         '<li>Click on "groestlcoin:" links in your web browser '
   ...
   def GetDashStateText(self, mgmtMode, state):
      ...
      if state == 'NewUserInfo':
         ...
         'will also put Groestlcoin Armory into offline mode for a few minutes '
         ...
         'Groestlcoin Armory in online mode, but any transactions associated with the '
         'new addresses will not appear in the ledger. '
         '<br><br>'
         'Pressing the button above will put Groestlcoin Armory into offline mode '
      ...
      if mgmtMode.lower()=='user':
         if state == 'OfflineButOnlinePossible':
            ...
            'Groestlcoin-Qt/groestlcoind is fully synchronized with the Groestlcoin network.  '
            ...
            'please visit the Groestlcoin Armory '
            ...
            return tr( \
            'You are currently in offline mode because '
            'Groestlcoin Armory could not find the blockchain files produced '
            'by Groestlcoin-Qt.  Do you run Groestlcoin-Qt (or groestlcoind) '
            'from a non-standard directory?   Groestlcoin Armory expects to '
            'find the blkXXXX.dat files in <br><br>%s<br><br> '
            'If you know where they are located, please restart '
            'Groestlcoin Armory using the " --satoshi-datadir=[path]" '
            'to notify Groestlcoin Armory where to find them.') % BLKFILE_DIR
         elif state == 'Disconnected':
            return tr( \
            'Groestlcoin Armory was previously online, but the connection to Groestlcoin-Qt/'
            ...
            'Groestlcoin Armory will go into online mode automatically, as soon as '
            'the scan is complete.')
         elif state == 'ScanWithWallets':
            return tr( \
            'Groestlcoin Armory is scanning the global transaction history to retrieve '
         ...
         if state == 'OfflineBadConnection':
            return tr( \
            'Groestlcoin Armory has experienced an issue trying to communicate with the '
            'Groestlcoin software.  The software is running in the background, '
            'but Groestlcoin Armory cannot communicate with it through RPC as it expects '
            ...
            '(from the "File" menu) and send it to groestlcoin@gmail.com')
         if state == 'OfflineSatoshiAvail':
            return tr( \
            'Groestlcoin Armory does not detect internet access, but it does detect '
            'running Groestlcoin software.  Groestlcoin Armory is in offline-mode. <br><br>'
         ...
         if state == 'OfflineForcedButSatoshiAvail':
            return tr( \
            'Groestlcoin Armory was started in offline-mode, but detected you are '
         ...
         if state == 'OfflineBadDBEnv':
            ...
            'Groestlcoin-Qt/groestlcoind is upgraded or downgraded, or sometimes '
         ...
         if state == 'OfflineGrsdCrashed':
            ...
            if len(sout)>0 or len(serr)>0:
               ...
               return ( tr("""
                  ...
                  groestlcoin@gmail.com?Subject=Groestlcoind%20Crash">
                  groestlcoin@gmail.com</a>.  We apologize for the
                  inconvenience!"""))
      ...
      if self.doAutoBitcoind and not sdmState=='GroestlcoindReady':
         ...
         if not sdmState==self.lastSDMState:
            ...
            if self.internetStatus == INTERNET_STATUS.Unavailable or CLI_OPTIONS.offline:
               ...
               setOnlyDashModeVisible()
               self.lblDashModeSync.setText( tr('Groestlcoin Armory is <u>offline</u>'), \
                                            size=4, color='TextWarn', bold=True)
               if satoshiIsAvailable():
                  self.frmDashMidButtons.setVisible(True)
                  setBtnRowVisible(DASHBTNS.Close, True)
                  if CLI_OPTIONS.offline:
                     # Forced offline but groestlcoind is running
               ...
               if satoshiIsAvailable() or sdmState=='GroestlcoindAlreadyRunning':
                  # But groestlcoind/-qt is already running
                  ...
                  setBtnFrameVisible(True, \
                     ...
                     '<a href="mailto:groestlcoin@gmail.com?Subject=Groestlcoind%20Crash"'
                     '>groestlcoin@gmail.com</a>.'))
                  setBtnRowVisible(DASHBTNS.Settings, True)
                  setBtnRowVisible(DASHBTNS.Install, True)
                  LOGINFO('Dashboard switched to auto-GrsdCrashed')
                  self.lblDashModeSync.setText( tr('Groestlcoin Armory is <u>offline</u>'), \
                                            size=4, color='TextWarn', bold=True)
                  descr1 += self.GetDashStateText('Auto', 'OfflineGrsdCrashed')
                  descr2 += self.GetDashFunctionalityText('Offline')
                  self.lblDashDescr1.setText(descr1)
                  self.lblDashDescr2.setText(descr2)
               ...
               if sdmState in ['GroestlcoindWrongPassword', 'GroestlcoindNotAvailable']:
                  ...
                  setBtnRowVisible(DASHBTNS.Settings, True)
                  setBtnFrameVisible(True, \
                     tr('Groestlcoin Armory runs groestlcoind in the background'))

                  descr2 += self.GetDashFunctionalityText('Offline')
                  self.lblDashDescr1.setText(descr1)
                  self.lblDashDescr2.setText(descr2)
      else:
         # User is managing satoshi client, or groestlcoind is already sync'd
         self.frmDashMidButtons.setVisible(False)
         if bdmState in (BDM_OFFLINE, BDM_UNINITIALIZED):
            if self.internetStatus == INTERNET_STATUS.Unavailable:
               ...
               setOnlyDashModeVisible()
               self.lblBusy.setVisible(False)
               self.lblDashModeSync.setText(tr('Groestlcoin Armory is <u>offline</u>'), size=4, bold=True)
               ...
               if not satoshiIsAvailable():
                  ...
                  setBtnRowVisible(DASHBTNS.Settings, True)
                  setBtnFrameVisible(True, \
                     tr('If you would like Groestlcoin Armory to manage the Groestlcoin software '
                     'for you (Groestlcoin-Qt or groestlcoind), then adjust your '
                     'Groestlcoin Armory settings, then restart Groestlcoin Armory.'))
                  descr = self.GetDashStateText('User','OfflineNoSatoshiNoInternet')
               else:
                  self.btnModeSwitch.setVisible(True)
            ...
            if self.netMode == NETWORKMODE.Disconnected:
               self.btnModeSwitch.setVisible(False)
               self.lblDashModeSync.setText( tr('Groestlcoin Armory is disconnected'), size=4, color='TextWarn', bold=True)
               ...
               # Fully online mode
               self.btnModeSwitch.setVisible(False)
               self.lblDashModeSync.setText( tr('Groestlcoin Armory is online!'), color='TextGreen', size=4, bold=True)
               self.mainDisplayTabs.setTabEnabled(self.MAINTABS.Ledger, True)
      ...
      try:
         ...
         if self.doAutoBitcoind:
            ...
            if bdmState in (BDM_OFFLINE,BDM_UNINITIALIZED):
               ...
               # mode, it needs to check periodically for the existence of Groestlcoin-Qt
   ...
   def closeEvent(self, event=None):
      ...
      if moc=='DontKnow':
         reply,remember = MsgBoxWithDNAA(self, self, MSGBOX.Question, tr('Minimize or Close'), \
            tr('Would you like to minimize Groestlcoin Armory to the system tray instead '
   ...
   def actuallyDoExitNow(self, action, l):
      ...
      # This will do nothing if groestlcoind isn't running.
      try:
...
def checkForAlreadyOpen():
   ...
   try:
      ...
      # first/only instance of Groestlcoin Armory and opening the socket will err out
      pass
...
def checkForAlreadyOpenError():
   ...
   for proc in psutil.process_iter():
      ...
      if bexe in pname:
         LOGINFO('Found groestlcoind PID: %d', proc.pid)
